...
import {  Folder, LogOut, FileText, GitBranch, ShieldCheck, FileStack, History } from "lucide-react";
...
export function AppSidebar({ collapsed, setCollapsed }: { collapsed: boolean; setCollapsed: (collapsed: boolean) => void; }) {
  return (
    <div style={{backgroundColor:"#4F46E5"}}className={`fixed top-0 left-0 h-screen  text-white border-r p-4 flex flex-col ${collapsed ? 'w-20' : 'w-64'} transition-all duration-300 z-50`}>
      {/* Toggle Collapse Button */}
      <div className="flex justify-between items-center mb-6">
        {!collapsed && <span className="text-xl font-bold">MyApp</span>}
        <Button
          variant="ghost"
          size="icon"
